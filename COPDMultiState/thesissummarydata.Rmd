...
copd2 <- copd_p1p2[c( 1,2,3,5,6,8:11,13,14,445,446,15, 22,23,30, 31, 37, 579:585, 587,588, 628, 629,631:644, 652:668, 678,682, 757, 758)] #dataset of repeated variables of interest
names(copd2)
...
names(copd3) <- c("sid", "visitnum", "finalgold1")
names(copd4) <- c("sid", "visitnum", "finalgold2")
#every subject id and every row needs to have finalgold1 and finalgold2 even if they are empty. 
copd3 <- copd3[,c(1,3)]
copd4 <- copd4[,c(1,3)] #drop the visit value


copd5 <- merge(copd2,copd3,by=c("sid"), all.x=TRUE)
copd6 <- merge(copd5,copd4, by=c("sid"), all.x=TRUE)


#I think i need to merge copd4 and copd3 in with the main copd2 dataset. This will require a merge, not a column bind because there are differing numbers of rows - we'll match based on subject id. 

#I want every row to have finalgold1 and finalgold2 so that I can use these for summary statistics. Conveniently, we'll also keep finalgold as a general variable that we can use for time-varying covariates if we want to.

library(dplyr)

#pull a group of items from each subject that don't change to be added to the LFU dataset

unchanging <- select(copd6,c("sid", "ccenter", "gender", "race", "ethnic", "EverSmokedCig", "ExclusionaryDisease", "age_baseline", "Income", "Insurance", "finalgold1", "finalgold2", "visitnum")) 
...
items <- unchanging %>% group_by(sid) %>% summarize(visitcount = max(visitnum)) 

check <- select(unchanging, c("sid", "visitnum"))
check2 <- merge(unchanging, items, by = "sid")  
...
total <- merge(check3,surveyrep,by=c("sid"))
...
LFUsurv <- LFUsurv[,-8] #drop ccenter because I think that complicates things.

rename(newDat, sid2 = sid)
names(newDat) #there are 14 columns here



LFUdescript <- cbind(LFUsurv, newDat)  ####        THIS IS THE SEMI-FINAL DATASET         ####

#check column heads on LFUdescript
intersect(names(LFUdescript), names(LFUsurv))
unique(names(LFUdescript), names(LFUsurv))

#sid is repeated on LFUdescript, also have visitnum, visitcount, drop, and counting . We can drop these before proceeding.
names(LFUdescript) 
#drop 32, 42, 42, 44, 45

LFUdescript <- LFUdescript[-c(32,44:47)]
names(LFUdescript) #now we're back down to 42 variables which is good
...
names(LFUdescript) #42 variables
names(copd6) #67 variables

fulldata2 <- merge(copd6,mort_adjud, by="sid")


 #fulldata has 123 variables, LFUdescript has 42 variables

#what's the number of shared variables between them?

intersect(names(LFUdescript), names(fulldata2)) #12 shared variables between fulldata and LFU description
setdiff(names(LFUdescript),names(fulldata2))
setdiff(names(fulldata2),names(LFUdescript))

empty <- data.frame(matrix(NA, nrow=17129, ncol=30)) #add to copd/mort
empty2 <- data.frame(matrix(NA, nrow=81912, ncol=111)) #add to LFUsurv

fulldata3 <- cbind(fulldata2, empty)

LFUdescript2 <- cbind(LFUdescript, empty2)

name1 <- names(LFUdescript2)#153


name2 <-names(fulldata3) #153



a <-c("sid", "surveyNum","SurveyYear","MonthsSinceInitialVisit","monthsSincePrevSurvey","Exclude_ILD_Bronchiectasis", "Exclude_LungTrans_LVRS" ,"SurveyVersion" ,"MethodTaken","Episode_sinceLast", "Antibiotics_SinceLast","Antibiotic_times","Steroids_sinceLast","Steroid_times" ,"Antib_steroid_sinceLast", "Antib_steroid_times","UrgentDr_SinceLast" ,"UrgentDr_times","ER_sinceLast", "ER_times"  , "Hosp_sinceLast",  "Hosp_times" ,"ICU_sinceLast","BreathingTube_sinceLast" , "NewMeds_sinceLast","O2_sinceLast" ,    "CurrentlySmoking_LFU" , "exacerb_Ster_Antib" , "episode_untreated"  ,"Net_Exacerbations",  "Net_Severe_Exac", "ccenter", "gender", "race", "ethnic","EverSmokedCig",       "ExclusionaryDisease", "age_baseline", "Income", "Insurance" , "finalgold1", "finalgold2", "visitnum", "age_visit","Height_CM" ,"Weight_KG","Waist_CM","Arm_Span_CM" ,"BMI" ,"BODE",  "ATS_PackYears","YearsSinceQuit", "Duration_Smoking","Severe_Exacerbations" ,"Exacerbation_Frequency" ,"Chronic_Bronchitis", "SafetyIssues_P2" ,  "ConfirmedNoCT","finalGold"  ,"finalgold_visit", "FEV1pp_utah","FVCpp_utah","FEV1_FVC_utah"  ,"FEV1_utah" ,"FEV6_utah", "FVC_utah", "PEF_utah","FEF2575_utah","pre_FEV1" ,"pre_FEV6","pre_FVC","pre_FEV1_FVC","pre_PEF" , "pre_FEF2575" , "finalgold_Phase2","NewGOLD_SGRQ","Chronic_Bronchitis_P1", "Days_Phase1_Phase2", "Years_Phase1_Phase2" , "Change_P1_P2_Smoking_Status", "Change_P1_P2_Gold_class", "Change_P1_P2_FEV1_ml","Change_P1_P2_FEV1pp" ,"Change_P1_P2_FEV1_ml_yr" , "Change_P1_P2_MMRC","Change_P1_P2_SGRQ_total", "Change_P1_P2_distwalked", "Change_P1_P2_O2","New_Chronic_Bronchitis","signif_lungchange_suppress", "smoking_status_change", "pctEmph_Thirona", "pctGasTrap_Thirona" ,"Pi10_Thirona" ,"AWT_seg_Thirona","vital_status","mortality_survival_vetted","mortality_survival_vital_status" , "months_followed_net" , "months_followed_visits", "months_followed_LFU", "months_followed_SSDI_search" ,"days_followed", "mortality_cohort" ,"SSDI_searched" ,   "death_cause_adjud","Torch_UCD" , "Torch_UCD_description" ,"Torch_UCD_Other_specify",  "Torch_Group_Basic", "Torch_Group_Basic_descrip", "Torch_UCD_COPD_related" ,"CCOD_COPD_Exac_w_Pneu", "CCOD_COPD_Exac_wo_Pneu",  "CCOD_COPD_no_exac", "CCOD_OthResp","CCOD_MI" , "CCOD_Stroke"  , "CCOD_DVT"  ,  "CCOD_OthCardiac", "CCOD_SuddenDeath",  "CCOD_SuddenCardiac", "CCOD_LungCancer" ,"CCOD_OthCancer","CCOD_Renal", "CCOD_Diabetes","CCOD_Sepsis",  "CCOD_Accdnt_suicd" ,"CCOD_OthDis" ,"CCOD_UnknownFactor","CCOD_Accdnt_Details",  "CCOD_COPD_resp",   "CCOD_CVD" ,"CCOD_Cancer","CCOD_Other" , "AnyCancerDx"   ,"CancerImpact","source_DeathCert" ,"source_InformInterview", "source_MedicalRec" , "source_Other", "source_None", "InfoAssessment","Comorbidity_CoronaryArtery",  "Comorbidity_LungCancer" , "Comorbidity_Osteopors",  "Comorbidity_AnxDepr","Comorbidity_Diabetes","AdjudUCD_MatchDeathCert_12" , "AdjudUCD_MatchDeathCert_any"  ,  "COPD_on_DeathCert" , "Visit_Year", "Visit_Date") #153, these are the names for LFUdescript2

setdiff(a, name1)

b <- c("sid","ccenter", "visitnum","Visit_Year", "Visit_Date", "gender" , "race","ethnic", "EverSmokedCig" ,"ExclusionaryDisease", "age_baseline","Income" ,"Insurance" , "age_visit",
"Height_CM" , "Weight_KG", "Waist_CM","Arm_Span_CM" , "BMI" , "BODE" , "ATS_PackYears", "YearsSinceQuit","Duration_Smoking","Severe_Exacerbations" ,"Exacerbation_Frequency"  ,"Chronic_Bronchitis" , "SafetyIssues_P2" ,"ConfirmedNoCT"  , "finalGold","finalgold_visit" , "FEV1pp_utah","FVCpp_utah" ,"FEV1_FVC_utah", "FEV1_utah" ,"FEV6_utah"  ,"FVC_utah" ,   "PEF_utah" ,"FEF2575_utah", "pre_FEV1" ,"pre_FEV6" ,"pre_FVC"  ,"pre_FEV1_FVC", "pre_PEF" ,"pre_FEF2575","finalgold_Phase2" ,"NewGOLD_SGRQ", "Chronic_Bronchitis_P1" , "Days_Phase1_Phase2","Years_Phase1_Phase2" ,"Change_P1_P2_Smoking_Status", "Change_P1_P2_Gold_class", "Change_P1_P2_FEV1_ml","Change_P1_P2_FEV1pp" ,"Change_P1_P2_FEV1_ml_yr" ,"Change_P1_P2_MMRC", "Change_P1_P2_SGRQ_total"  ,"Change_P1_P2_distwalked" ,"Change_P1_P2_O2", "New_Chronic_Bronchitis",  "signif_lungchange_suppress" ,"smoking_status_change" ,"pctEmph_Thirona", "pctGasTrap_Thirona", "Pi10_Thirona","AWT_seg_Thirona", "finalgold1", "finalgold2","vital_status"  ,"mortality_survival_vetted","mortality_survival_vital_status" ,"months_followed_net" , "months_followed_visits","months_followed_LFU", "months_followed_SSDI_search" ,"days_followed", "mortality_cohort" , "SSDI_searched",  "death_cause_adjud" ,  "Torch_UCD",    "Torch_UCD_description"  ,"Torch_UCD_Other_specify", "Torch_Group_Basic", "Torch_Group_Basic_descrip","Torch_UCD_COPD_related" , "CCOD_COPD_Exac_w_Pneu","CCOD_COPD_Exac_wo_Pneu", "CCOD_COPD_no_exac" , "CCOD_OthResp","CCOD_MI", "CCOD_Stroke","CCOD_DVT","CCOD_OthCardiac","CCOD_SuddenDeath", "CCOD_SuddenCardiac","CCOD_LungCancer","CCOD_OthCancer","CCOD_Renal" ,                     "CCOD_Diabetes","CCOD_Sepsis" , "CCOD_Accdnt_suicd","CCOD_OthDis" ,"CCOD_UnknownFactor" ,"CCOD_Accdnt_Details" ,"CCOD_COPD_resp" ,"CCOD_CVD",  "CCOD_Cancer" , "CCOD_Other","AnyCancerDx","CancerImpact" ,  "source_DeathCert"  ,"source_InformInterview"  ,"source_MedicalRec" , "source_Other", "source_None" , "InfoAssessment","Comorbidity_CoronaryArtery", "Comorbidity_LungCancer"  ,"Comorbidity_Osteopors",  "Comorbidity_AnxDepr",  "Comorbidity_Diabetes", "AdjudUCD_MatchDeathCert_12" ,"AdjudUCD_MatchDeathCert_any","COPD_on_DeathCert" , "surveyNum","SurveyYear","MonthsSinceInitialVisit" ,"monthsSincePrevSurvey","Exclude_ILD_Bronchiectasis","Exclude_LungTrans_LVRS" , "SurveyVersion","MethodTaken" ,"Episode_sinceLast","Antibiotics_SinceLast" ,"Antibiotic_times", "Steroids_sinceLast","Steroid_times", "Antib_steroid_sinceLast","Antib_steroid_times" , "UrgentDr_SinceLast", "UrgentDr_times", "ER_sinceLast","ER_times","Hosp_sinceLast" ,"Hosp_times" ,"ICU_sinceLast", "BreathingTube_sinceLast" ,"NewMeds_sinceLast" ,"O2_sinceLast","CurrentlySmoking_LFU" ,"exacerb_Ster_Antib" ,"episode_untreated","Net_Exacerbations","Net_Severe_Exac") #149

setdiff(b, name2)
#setdiff(b,a) #check that they have the same column names

names(LFUdescript2) <- a
...
testing <- rbind(LFUdescript2, fulldata3)
...
mort_adjud$COPD_Exac_w_Pneu <- ifelse(mort_adjud$COPD_Exac_w_Pneu == 1, "COPD_Exac_w_Pneu",mort_adjud$COPD_Exac_w_Pneu )

mort_adjud$COPD_Exac_wo_Pneu <- ifelse(mort_adjud$COPD_Exac_wo_Pneu == 1, "COPD_Exac_wo_Pneu",mort_adjud$COPD_Exac_wo_Pneu)

mort_adjud$COPD_no_exac <- ifelse(mort_adjud$COPD_no_exac == 1, "COPD_no_exac",mort_adjud$COPD_no_exac)

mort_adjud$OthResp <- ifelse(mort_adjud$OthResp == 1, "OthResp",mort_adjud$OthResp)

mort_adjud$MI <- ifelse(mort_adjud$MI == 1, "MI",mort_adjud$MI)

mort_adjud$Stroke <- ifelse(mort_adjud$Stroke == 1, "Stroke",mort_adjud$Stroke)

mort_adjud$DVT <- ifelse(mort_adjud$DVT == 1, "DVT",mort_adjud$DVT)

mort_adjud$OthCardiac <- ifelse(mort_adjud$OthCardiac == 1, "OthCardiac",mort_adjud$OthCardiac)

mort_adjud$SuddenDeath <- ifelse(mort_adjud$SuddenDeath== 1, "SuddenDeath",mort_adjud$SuddenDeath)

mort_adjud$SuddenCardiac <- ifelse(mort_adjud$SuddenCardiac == 1, "SuddenCardiac",mort_adjud$SuddenCardiac)

mort_adjud$LungCancer <- ifelse(mort_adjud$LungCancer== 1, "LungCancer",mort_adjud$LungCancer)

mort_adjud$OthCancer <- ifelse(mort_adjud$OthCancer == 1, "OthCancer",mort_adjud$OthCancer)

mort_adjud$Renal <- ifelse(mort_adjud$Renal== 1, "Renal",mort_adjud$Renal)

mort_adjud$Diabetes <- ifelse(mort_adjud$Diabetes == 1, "Diabetes",mort_adjud$Diabetes)

mort_adjud$Sepsis <- ifelse(mort_adjud$Sepsis == 1, "Sepsis",mort_adjud$Sepsis)

mort_adjud$Accdnt_suicd <- ifelse(mort_adjud$Accdnt_suicd == 1, "Accdnt_suicd",mort_adjud$Accdnt_suicd)

mort_adjud$OthDis <- ifelse(mort_adjud$OthDis == 1, "OthDis",mort_adjud$OthDis)

mort_adjud$UnknownFactor <- ifelse(mort_adjud$UnknownFactor == 1, "UnknownFactor",mort_adjud$UnknownFactor)

mort_adjud$COPD_resp <- ifelse(mort_adjud$COPD_resp == 1, "COPD_resp",mort_adjud$COPD_resp)

mort_adjud$CVD <- ifelse(mort_adjud$CVD == 1, "CVD",mort_adjud$CVD)

mort_adjud$Cancer <- ifelse(mort_adjud$Cancer== 1, "Cancer",mort_adjud$Cancer)

mort_adjud$Other <- ifelse(mort_adjud$Other == 1, "Other",mort_adjud$Other)

mort_adjud$Accdnt_Details <- as.factor(mort_adjud$Accdnt_Details)

levels(mort_adjud$Accdnt_Details)[levels(mort_adjud$Accdnt_Details)=="1"] <- "accident"
levels(mort_adjud$Accdnt_Details)[levels(mort_adjud$Accdnt_Details)=="2"] <- "suicide"
levels(mort_adjud$Accdnt_Details)[levels(mort_adjud$Accdnt_Details)=="3"] <- "drug"
