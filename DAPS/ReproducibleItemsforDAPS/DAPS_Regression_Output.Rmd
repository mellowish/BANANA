...
endodf <- daps1[,c(1,3,5:7,8:26,38:43,44,45,46,55)] #select covariates, outcomes, and endotoxins from visit 1 values
...
p <- ldply(endodf[c(6:7,17:33)], function(x) { #used ldply because it returns dataframes which is convenient given that I'm stacking all of these.
...
Glucan
```{r}
glucdf <- daps1[,c(1,3,5:7,8:26,38:46,56)]#select covariates, outcomes, and glucans from visit 1 values



#for non-spirometrics
p <- ldply(glucdf[c(6:7,17:33)], function(x) {
...
```{r}
vozodf <- daps_s[,c(1,3,5:7,8:26,38:46,31)] #select covariates, outcomes, and ozone values from summer visits only. This is the personal monitor ozone data first.

#get a linear model for each outcome, apply the lsmeans function for the exposure and extract the lsmean/sd error value

#for non-spirometrics

p <- ldply(vozodf[c(6:7,17:33)], function(x) {
...
IQR_exp <-rep(iqr,(nrow(p)+nrow(q)))
vest <- cbind(vest,IQR_exp)
vest$Rel_Val <- vest$Estimate/iqr



pop_ozodf <- daps_s[,c(1,3,5:7,8:26,38:46,36)] #select covariates, outcomes, and ozone values from summer visits only. This is the Denver CDPHE data for ozone.


#for non-spirometrics

p1 <- ldply(pop_ozodf[c(6:7,17:33)], function(x) {
...
IQR_exp <-rep(iqr2,(nrow(p1)+nrow(q1)))
...
## Repeated Measures ##
```{r}
vpm10df <- daps[, c(1,3,5:7, 8:18, 38:41,46,48, 27)]
spm10df <- daps[, c(1,3,5:7, 8:18, 38:41,46,48, 28)]
phepm10df <- daps[, c(1,3,5:7, 8:18, 38:41,46,48, 32)]
vno2df <- daps[, c(1,3,5:7, 8:18, 38:41,46,48, 29)]
sno2df <- daps[, c(1,3,5:7, 8:18, 38:41,46,48, 30)]
pheno2df <- daps[, c(1,3,5:7, 8:18, 38:41,46,48, 34)]
nicdf <- daps[, c(1,3,5:7, 8:18, 38:41,46,48, 49)]
...
p <- ldply(vpm10df[c(6:7,17:22)], function(x) {
...
p1 <- ldply(spm10df[c(6:7,17:22)], function(x) {
...
p2 <- ldply(phepm10df[c(6:7,17:22)], function(x) {
...
p <- ldply(vno2df[c(6:7,17:22)], function(x) {
...
p1 <- ldply(sno2df[c(6:7,17:22)], function(x) {
...
p2 <- ldply(pheno2df[c(6:7,17:22)], function(x) {
...
p <- ldply(nicdf[c(6:7,17:22)], function(x) {
...
mod.names4 <- mod.names4[c(3:31)] #get rid of the two blank ones
...
a <-  data.frame(matrix(NA, nrow=nrow(output3), ncol=11))
for (i in 1:11){
a[i] <- as.data.frame(str_match(output3$Model.Structure, mod.names5[i]))
}
...
OUTPUT1$count <- ifelse(OUTPUT1$Components.of.Variance.Correlation== "Residual", 1, 0)
OUTPUT2 <- subset(OUTPUT1,OUTPUT1$count !=1)
OUTPUT2$Residual.SD.Model <- sqrt(OUTPUT2$Residual.Variance.of.Model)
OUTPUT2 <- OUTPUT2[,-12]
names(OUTPUT2) <- c("OUTCOME","Exposure.Coefficient","Std..Error.Coef.Exp","Residual.Variance.of.Model","Random.Effect.Label","Rand.Effect.Var", "Rand.Effect.SD","Model.Structure","IQR.of.Exposure","Coef.IQR","Exposure","Residual.SD.Model")



OUTPUT3 <- OUTPUT2[,c( 1,11,2,3,4,12,5,6,7,8,9,10)]
